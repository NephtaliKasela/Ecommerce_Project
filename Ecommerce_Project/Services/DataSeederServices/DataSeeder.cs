    ...
    public class DataSeeder : IDataSeeder
    {
        ...
        public async Task SeedProducts(int numberOfProducts)
        {
            for (int i = 1; i <= numberOfProducts; i++)
            {
                ...
                .RuleFor(p => p.Price, f => Math.Round(f.Random.Decimal(90, 100), 2))
                .RuleFor(p => p.SoldPrice, f => Math.Round(f.Random.Decimal(50, 85), 2))
                ...
                .RuleFor(p => p.SerialNumber, f => Convert.ToString(f.Random.Decimal(1000000000, 1000000000000)))
